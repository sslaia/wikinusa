...
class LanguageCourseScreen extends ConsumerStatefulWidget {
  const LanguageCourseScreen({super.key});

  @override
  ConsumerState<LanguageCourseScreen> createState() =>
      _LanguageCourseScreenState();
}

class _LanguageCourseScreenState extends ConsumerState<LanguageCourseScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.invalidate(courseApiProvider);
    });
  }

  Future<void> _refreshCourse() async {
    final currentLanguage = ref.read(languageProvider);
    final courseConfig = ref.read(
    ...
    await WikiApiService.clearCache(project, currentLanguage, courseTitle);
  ...
  Widget build(BuildContext context) {
    ...
    final isEnabled = courseConfig?.enabled ?? false;
    final courseTitle = (courseConfig?.pageTitle.isNotEmpty ?? false)
        ? courseConfig!.pageTitle
        : 'Wikikamus:Sulu';
    final project = courseConfig?.project ?? ProjectType.wiktionary;

    final courseContent = isEnabled
        ? ref.watch(
            courseApiProvider((
              pageTitle: courseTitle,
        ...
        return Scaffold(
          ...
          body: Row(
            children: [
              ...
              Expanded(
                child: !isEnabled
                    ? _buildDisabledState(mixedColor)
                    : Stack(
                        children: [
                          CustomScrollView(
                            slivers: [
                              ...
                              if (courseContent != null)
                                courseContent.when(
                                  data: (data) {
                                    ...
                                    // Parse and extract specific lesson components
                                    ...
                                    // Remove title from document to prevent duplicate rendering
                                    titleElement?.remove();

                                    // Surgical content extraction: prefer .lesson-content, then .mw-parser-output, then body.
                                    String cleanBody;
                                    if (contentElement != null) {
                                      cleanBody = contentElement.innerHtml
                                          .trim();
                                    } else {
                                      final mainContent =
                                          doc.querySelector(
  ...
  Widget _buildDisabledState(Color color) {
    return Scaffold(
      appBar: AppBar(
        title: Text('language_course'.tr()),
      ),
      body: ModuleDisabledView(
        moduleTitle: 'language_course'.tr(),
  ...
  Future<bool> _handleUrlTap(
    ...
    if (url.startsWith('http://') || url.startsWith('https://')) {
      ...
      targetUri = Uri.tryParse('https:$url');
    } else if (url.startsWith('/')) {
      final domain = WikiConfig.getDomain(langCode, project.name.toLowerCase());
      targetUri = Uri.tryParse('https://$domain$url');
    } else {
      final domain = WikiConfig.getDomain(langCode, project.name.toLowerCase());
      targetUri = Uri.tryParse('https://$domain/wiki/$url');
    }

    if (targetUri != null) {
      try {
        await launchUrl(targetUri, mode: LaunchMode.inAppBrowserView);
        return true;
      } catch (e) {
        if (mounted) {
...
typedef CourseParams = ({
  String langCode,
  String pageTitle,
  ProjectType project,
});

// Special provider for course data dynamically configured per language with offline database caching
final courseApiProvider = FutureProvider.autoDispose.family<dynamic, CourseParams>((
  ref,
  params,
) async {
